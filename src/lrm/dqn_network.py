...
def create_linear_regression(x, num_input, num_output):
    # random init to handle ties in argmax
    W = tf.get_variable("w", [num_input, num_output], initializer=tf.random_uniform_initializer(minval=1.0, maxval=1.001, dtype=tf.float64), dtype=tf.float64)
...
def _add_dense_layer(x, num_input, num_output, add_relu):
    W = tf.get_variable("w", [num_input, num_output], initializer=tf.truncated_normal_initializer(mean=0.0, stddev=0.1, dtype=tf.float64), dtype=tf.float64)
